...
func TestAlipayPassTemplateAdd(t *testing.T) {
	client, _ := NewClient()
	data := request.AlipayPassTemplateAddRequest{
		UniqueId:   "20180828180000",
	...
	response, err := client.SendRequest(request.AlipayPassTemplateAddMethod, data)
	if err != nil {
		panic(err)
	}
	t.Log(response)
